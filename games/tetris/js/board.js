class Board {
    ...
    reset() {
        ...
        for (let i = 0; i < NEXT_PIECE_COUNT; i++) {
            this.nextPieces.push(new Piece(this.nextContexts[i]));
        }
        this.getNewPiece();
    }
    ...
    initNextQueue() {
        ...
        for (let i = 0; i < NEXT_PIECE_COUNT; i++) {
            ...
            this.nextQueueElement.appendChild(canvas);
            this.nextContexts.push(canvas.getContext('2d'));
        }
    }
    ...
    drawNextQueue() {
        this.nextPieces.forEach((piece, index) => {
            const ctx = this.nextContexts[index];
    ...
    getNewPiece() {
        ...
        if (this.piece) this.piece.ctx = this.ctx;
        this.piece.setStartingPosition();
        this.nextPieces.push(new Piece(this.nextContexts[NEXT_PIECE_COUNT - 1]));
        this.canHold = true;
    }
    ...
    // ... Other stable methods like freeze, hold, isValid, etc. ...
    getEmptyGrid() { return Array.from({ length: ROWS }, () => Array(COLS).fill(0)); }
    drawGrid() { this.ctx.strokeStyle = getComputedStyle(document.documentElement).getPropertyValue('--grid-line-color'); this.ctx.lineWidth = 1; for (let i = 1; i < COLS; i++) { this.ctx.beginPath(); this.ctx.moveTo(i * BLOCK_SIZE, 0); this.ctx.lineTo(i * BLOCK_SIZE, this.ctx.canvas.height); this.ctx.stroke(); } for (let i = 1; i < ROWS; i++) { this.ctx.beginPath(); this.ctx.moveTo(0, i * BLOCK_SIZE); this.ctx.lineTo(this.ctx.canvas.width, i * BLOCK_SIZE); this.ctx.stroke(); } }
    drawPieceOnSideCanvas(context, piece) { context.clearRect(0, 0, context.canvas.width, context.canvas.height); if (piece) {piece.ctx = context; piece.draw(BLOCK_SIZE, true);} }
