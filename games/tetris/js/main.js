document.addEventListener('DOMContentLoaded', () => {
    ...
    function hardDrop() {
        if (isAnimatingHardDrop || !board.piece) return;
        const startY = board.piece.y;
        let p = { ...board.piece };
        while (board.isValid(p)) p.y++;
        p.y--;
        const endY = p.y;
        if (endY <= startY) { drop(); return; }
    ...
    canvas.addEventListener('touchstart', e => { if (isAnimatingHardDrop) return; e.preventDefault(); touchStartX = e.changedTouches[0].screenX; touchStartY = e.changedTouches[0].screenY; }, { passive: false });
